...
export default async function GoalsPage() {
  ...
  const { settings, latestMetric, latestRun, sevenDayAverage, publishedToday, categories, insights, sources, experiments } = dashboard;
    ...
    <div className="flex flex-col gap-8">
      ...
      <section className="grid gap-5 lg:grid-cols-[minmax(0,1.25fr)_minmax(320px,.75fr)]">
        ...
        <aside className="flex flex-col gap-5">
          <section><h2 className="text-body font-semibold">Source priorities</h2><div className="mt-3 divide-y divide-line rounded-lg border border-line bg-surface">{sources.map((source, index) => <div key={source.id} className="p-4"><div className="flex items-center justify-between gap-3"><p className="text-meta font-semibold">{index + 1}. {source.name}</p><span className="text-[12px]">{source.agentPriority.toFixed(1)}</span></div><p className="mt-1 text-[11px] leading-relaxed text-ink-muted">{source.agentReason ?? "Awaiting the first learning run."}</p></div>)}</div></section>
