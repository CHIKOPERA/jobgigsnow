...
- **New jobs land at `Job.status = "READY"`, not `"PUBLISHED"`** — a safety default for a new
  automated pipeline. Promote via the already-documented `GET /api/ingest/queue?stage=posting`
  flow. One line to change if full auto-publish is wanted instead.
